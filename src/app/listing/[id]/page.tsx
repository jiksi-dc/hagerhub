  ...
  async function submitComment() {
    ...
    const res = await fetch('/api/comments', {
      ...
      body: JSON.stringify({
        ...
        is_seller_reply: currentUserId === listing?.user_id,
      }),
    })
    if (res.ok) {
      setCommentBody('')
      ...
      <div style={{padding:'20px'}}>
        ...
        <div style={{borderTop:'2px solid #f0f0f0',paddingTop:'24px'}}>
          <div style={{display:'flex',alignItems:'center',gap:'8px',marginBottom:'20px'}}>
            <h2 style={{fontSize:'18px',fontWeight:'800',margin:0}}>Comments &amp; Reviews</h2>
            <span style={{background:'#f0f0f0',borderRadius:'20px',padding:'2px 10px',fontSize:'13px',color:'#666'}}>{comments.length}</span>
          </div>

          {commentsLoading ? (
            <div style={{color:'#999',fontSize:'14px',marginBottom:'20px'}}>Loading comments...</div>
          ) : comments.length === 0 ? (
            <div style={{color:'#999',fontSize:'14px',marginBottom:'20px'}}>No comments yet. Be the first to leave a review.</div>
          ) : (
            <div style={{display:'flex',flexDirection:'column',gap:'16px',marginBottom:'24px'}}>
              {comments.map(c => (
                <div key={c.id} style={{display:'flex',gap:'12px',background:c.is_seller_reply?'#fffbeb':'#fafafa',border:c.is_seller_reply?'1px solid #fcd34d':'1px solid #f0f0f0',borderRadius:'12px',padding:'14px'}}>
                  <div style={{width:'36px',height:'36px',borderRadius:'50%',background:'#078754',color:'white',display:'flex',alignItems:'center',justifyContent:'center',fontWeight:'700',fontSize:'14px',flexShrink:0}}>
                    {c.is_seller_reply ? 'S' : (c.profiles?.full_name?.[0]?.toUpperCase() ?? '?')}
                  </div>
                  <div style={{flex:1}}>
                    <div style={{display:'flex',alignItems:'center',gap:'8px',marginBottom:'4px',flexWrap:'wrap'}}>
                      <span style={{fontWeight:'700',fontSize:'14px'}}>{c.is_seller_reply ? 'Seller' : (c.profiles?.full_name ?? 'User')}</span>
                      {c.is_seller_reply && <span style={{background:'#f59e0b',color:'white',borderRadius:'4px',padding:'1px 6px',fontSize:'11px',fontWeight:'700'}}>SELLER</span>}
                      <span style={{color:'#999',fontSize:'12px',marginLeft:'auto'}}>{formatDate(c.created_at)}</span>
                    </div>
                    <p style={{margin:0,fontSize:'14px',color:'#444',lineHeight:1.5}}>{c.body}</p>
                  </div>
                </div>
              ))}
            </div>
          ...
          {currentUserId ? (
            <div style={{borderTop:'1px solid #f0f0f0',paddingTop:'16px'}}>
              ...
              <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginTop:'8px'}}>
                <span style={{fontSize:'12px',color:'#999'}}>{commentBody.length}/500</span>
                {commentError && <span style={{fontSize:'12px',color:'#ef4444'}}>{commentError}</span>}
                <button onClick={submitComment} disabled={submitting || !commentBody.trim()}
            ...
            <div style={{borderTop:'1px solid #f0f0f0',paddingTop:'16px',fontSize:'14px',color:'#666'}}>
              <a href="/auth/login" style={{color:'#078754',fontWeight:'700',textDecoration:'none'}}>Sign in</a> to leave a comment or review.
            </div>
          )}
        </div>
      </div>
