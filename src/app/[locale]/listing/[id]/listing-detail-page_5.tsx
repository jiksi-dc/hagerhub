...
if (data?.id) {
          supabase.auth.getUser().then(({ data: u }) => {
            if (!u?.user) { setSellerPhone(''); return }
            supabase.from('listings').select('contact_phone').eq('id', data.id).single()
              .then(({ data: c }) => setSellerPhone((c as any)?.contact_phone || ''))
          })
        }
      })
  ...
  const toggleSave = async () => {
    if (!user) { window.location.href = `/${locale}/login?next=/${locale}/listing/${id}`; return }
    const supabase = createClient()
    if (saved) {
            ...
            {sellerPhone ? (
              ...
              user ? (
                ...
                <div style={{textAlign:'center',padding:'20px'}}>
                  <div style={{fontSize:'14px',color:'#6B7280',marginBottom:'14px'}}>Sign in to see the seller&apos;s contact details.</div>
                  <a href={`/${locale}/login?next=/${locale}/listing/${id}`}
                    style={{display:'block',width:'100%',padding:'12px',background:'#111',color:'#fff',borderRadius:'10px',fontSize:'14px',fontWeight:700,textDecoration:'none'}}>
                    Sign in to call
                  </a>
                </div>
              )
        ...
        <div style={{display:'flex',flexDirection:'column',gap:'14px'}}>
          ...
          {listing.amenities && listing.amenities.length > 0 && (
            <div style={{background:'#fff',borderRadius:'14px',border:'1px solid #F3F4F6',padding:'20px',marginBottom:'16px'}}>
              <div style={{fontSize:'12px',fontWeight:700,textTransform:'uppercase',letterSpacing:'1px',color:'#9CA3AF',marginBottom:'14px'}}>{listing.category === 'Vehicles' ? 'Features' : 'Amenities'}</div>
        ...
        <div style={{background:'#fff',borderRadius:'14px',border:'1px solid #F3F4F6',padding:'20px'}}>
          ...
          {user ? (
            ...
            <div style={{textAlign:'center',padding:'16px',borderTop:comments.length?'1px solid #F3F4F6':'none',paddingTop:comments.length?'16px':'0'}}>
              <a href={`/${locale}/login?next=/${locale}/listing/${id}`} style={{color:'#2563EB',fontSize:'13px',fontWeight:600}}>Sign in to leave a review →</a>
            </div>
          )}
        </div>
      </div>
