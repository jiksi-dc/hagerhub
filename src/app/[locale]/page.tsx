...
@keyframes featuredshimmer { 0%,100%{box-shadow:0 0 0 rgba(218,165,32,0)} 50%{box-shadow:0 4px 20px rgba(218,165,32,0.15)} }
      ...
      <section className="hero-section" style={{position:'relative',width:'100%',minHeight:'340px',background:'#f9fafb',overflow:'hidden'}}>
        ...
        <div className="hero-video" style={{flex:'0 0 auto',width:'640px',height:'360px',borderRadius:'12px',overflow:'hidden',position:'relative',background:'#000'}}>
          <video src="/Ethiofy final ad.mp4" autoPlay muted={adMuted} loop playsInline style={{width:'100%',height:'100%',objectFit:'cover',display:'block'}} /><button onClick={()=>setAdMuted(m=>!m)} aria-label={adMuted?'Unmute ad':'Mute ad'} style={{position:'absolute',bottom:'12px',right:'12px',width:'38px',height:'38px',borderRadius:'50%',border:'none',background:'rgba(0,0,0,0.55)',color:'#fff',fontSize:'17px',cursor:'pointer',display:'flex',alignItems:'center',justifyContent:'center'}}>{adMuted?'\u{1F507}':'\u{1F50A}'}</button>
        </div>
        </div>
      </section>
