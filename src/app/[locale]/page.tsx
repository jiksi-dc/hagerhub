...
const DICT: Record<string, Record<string, string>> = {
  en: { search:'Search properties, cars, jobs...', latest:'Latest Listings', browse:'Browse Categories', available:'listings available', loading:'Loading listings...', none:'No listings found', viewAll:'View all', view:'View →', seeAll:'See all →', ad:'ADVERTISEMENT', all:'All', properties:'Properties', vehicles:'Vehicles', machinery:'Machinery', classifieds:'Classifieds', jobs:'Jobs', back:'← All' },
  am: { search:'ቤቶች፣ መኪናዎች፣ ሥራዎች ይፈልጉ...', latest:'የቅርብ ጊዜ ዝርዝሮች', browse:'ምድቦችን ያስሱ', available:'ዝርዝሮች አሉ', loading:'ዝርዝሮች እየተጫኑ ነው...', none:'ምንም ዝርዝሮች አልተገኙም', viewAll:'ሁሉንም ይመልከቱ', view:'ይመልከቱ →', seeAll:'ሁሉንም ይመልከቱ →', ad:'ማስታወቂያ', all:'ሁሉም', properties:'ቤቶች', vehicles:'መኪናዎች', machinery:'ማሽነሪ', classifieds:'ዕቃዎች', jobs:'ሥራዎች', back:'← ሁሉም' },
  ar: { search:'ابحث عن عقارات، سيارات، وظائف...', latest:'أحدث القوائم', browse:'تصفح الفئات', available:'قوائم متاحة', loading:'جار تحميل القوائم...', none:'لم يتم العثور على قوائم', viewAll:'عرض الكل', view:'عرض →', seeAll:'عرض الكل →', ad:'إعلان', all:'الكل', properties:'عقارات', vehicles:'سيارات', machinery:'معدات', classifieds:'إعلانات', jobs:'وظائف', back:'← الكل' },
  fr: { search:'Rechercher des propriétés, voitures, emplois...', latest:'Dernières annonces', browse:'Parcourir les catégories', available:'annonces disponibles', loading:'Chargement des annonces...', none:'Aucune annonce trouvée', viewAll:'Voir tout', view:'Voir →', seeAll:'Voir tout →', ad:'PUBLICITÉ', all:'Tout', properties:'Propriétés', vehicles:'Véhicules', machinery:'Machines', classifieds:'Annonces', jobs:'Emplois', back:'← Tout' },
}
...
const IMGS: Record<string, string[]> = {
  Properties: ['https://images.unsplash.com/photo-1512917774080-9991f1c4c750?w=600&q=85','https://images.unsplash.com/photo-1570129477492-45c003edd2be?w=600&q=85','https://images.unsplash.com/photo-1600585154340-be6161a56a0c?w=600&q=85'],
  Vehicles: ['https://images.unsplash.com/photo-1533473359331-0135ef1b58bf?w=600&q=85','https://images.unsplash.com/photo-1494976388531-d1058494cdd8?w=600&q=85','https://images.unsplash.com/photo-1555215695-3004980ad54e?w=600&q=85'],
...
export default function Home() {
  ...
  return (
    <main style={{fontFamily:'inherit',background:'#F8FAFF',minHeight:'100vh',width:'100%',overflowX:'hidden'}}>
      <section style={{background:'#F8FAFF',padding:'28px 16px 24px',textAlign:'center',position:'relative',overflow:'visible'}}>
        ...
        <div style={{display:'flex',background:'#F8FAFF',borderRadius:'14px',overflow:'hidden',boxShadow:'0 2px 12px rgba(0,0,0,0.1)',border:'1.5px solid #eee',marginBottom:'16px'}}>
          ...
          <button onClick={fetchListings} style={{background:'#60A5FA',border:'none',color:'white',padding:'15px 22px',fontSize:'14px',fontWeight:800,cursor:'pointer',whiteSpace:'nowrap'}}>SEARCH</button>
        </div>
      </section>
      <div style={{background:"white",padding:"10px 16px",display:"flex",gap:"10px",alignItems:"center",borderBottom:"1px solid #eee"}}><AuthButton /><a href="/post" style={{background:"#60A5FA",color:"white",padding:"8px 18px",borderRadius:"20px",fontSize:"13px",fontWeight:700,textDecoration:"none"}}>+ Post Ad</a></div>
        ...
        <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',padding:'12px 16px',gap:'12px'}}>
          <div style={{display:'flex',alignItems:'center',gap:'12px'}}>
            <div style={{width:'48px',height:'48px',borderRadius:'50%',flexShrink:0,position:'relative',overflow:'hidden',border:'3px solid rgba(255,255,255,0.3)'}}>
              <div style={{position:'absolute',top:0,left:0,right:0,height:'50%',background:'#c00'}}/><div style={{position:'absolute',bottom:0,left:0,right:0,height:'50%',background:'#001489'}}/><div style={{position:'absolute',top:'46%',left:0,right:0,height:'8%',background:'#F8FAFF'}}/>
            </div>
          ...
          <div style={{display:'flex',alignItems:'center',gap:'12px'}}><div style={{fontSize:'36px'}}>🥤</div><div style={{background:'linear-gradient(135deg,#EF2118,#ff5544)',color:'white',fontSize:'12px',fontWeight:900,padding:'10px 18px',borderRadius:'20px',whiteSpace:'nowrap'}}>Win Prizes!</div></div>
        </div>
        <div style={{fontSize:'8px',color:'rgba(255,255,255,0.2)',textAlign:'center',padding:'2px 0 5px',letterSpacing:'2px'}}>{tx.ad}</div>
      </div>

      <div style={{display:'flex',overflowX:'auto',background:'#F8FAFF',boxShadow:'0 2px 10px rgba(0,0,0,0.07)',justifyContent:'center'}}>
      ...
      <div style={{background:'linear-gradient(90deg,#CC4A00,#FF7A00)',padding:'13px 16px',display:'flex',alignItems:'center',justifyContent:'space-between',cursor:'pointer',gap:'12px'}}>
        <div style={{display:'flex',alignItems:'center',gap:'12px'}}>
          <div style={{width:'42px',height:'42px',borderRadius:'12px',background:'#F8FAFF',display:'flex',alignItems:'center',justifyContent:'center',flexShrink:0,fontSize:'22px'}}>📱</div>
        ...
        <div style={{background:'#F8FAFF',color:'#CC4A00',fontSize:'12px',fontWeight:900,padding:'10px 18px',borderRadius:'20px',whiteSpace:'nowrap',flexShrink:0}}>Get App</div>
      </div>

      {activeCat==='All' && (
        <section style={{background:'#F8FAFF',padding:'22px 16px',borderBottom:'8px solid #F0F2F5'}}>
          <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:'18px'}}>
            <h2 style={{fontSize:'20px',fontWeight:900,color:'#111',margin:0}}>{tx.browse}</h2>
            <span style={{fontSize:'13px',color:'#60A5FA',fontWeight:700,cursor:'pointer'}}>{tx.seeAll}</span>
          </div>
          <div style={{display:'grid',gridTemplateColumns:'repeat(2,1fr)',gap:'12px'}}>
            {POPULAR.map(cat=>(
              <div key={cat.name} onClick={()=>setActiveCat(cat.name)} style={{background:'#F8FAFF',border:'1px solid #c7d2fe',borderRadius:'16px',padding:'16px 14px',cursor:'pointer'}}>
      ...
      <section style={{padding:'18px 16px'}}>
        <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:'18px'}}>
          ...
          {activeCat!=='All' && (<button onClick={()=>setActiveCat('All')} style={{fontSize:'12px',color:'#60A5FA',background:'#F8FAFF',border:'1.5px solid #0057D9',borderRadius:'20px',padding:'8px 18px',cursor:'pointer',fontWeight:700}}>{tx.back}</button>)}
        </div>
            ...
            {filtered.map((l,i)=>(
              <>
                {i===2 && <div key="ad1" style={{gridColumn:'1/-1',background:'linear-gradient(135deg,#001A6E,#0041C4)',borderRadius:'18px',padding:'18px',display:'flex',alignItems:'center',gap:'14px',cursor:'pointer',boxShadow:'0 8px 28px rgba(0,26,110,0.4)'}}>
                  <div style={{width:'56px',height:'56px',borderRadius:'16px',background:'#F8FAFF',display:'flex',alignItems:'center',justifyContent:'center',flexShrink:0,fontSize:'28px'}}>🏦</div>
                  ...
                  <div style={{background:'#60A5FA',color:'#001A6E',fontSize:'12px',fontWeight:900,padding:'12px 16px',borderRadius:'14px',whiteSpace:'nowrap',flexShrink:0}}>Apply Now</div>
                </div>}
                {i===5 && <div key="ad2" style={{gridColumn:'1/-1',borderRadius:'18px',overflow:'hidden',cursor:'pointer',boxShadow:'0 8px 28px rgba(0,0,0,0.2)',position:'relative'}}>
                  ...
                  <div style={{position:'absolute',inset:0,background:'linear-gradient(to right,rgba(0,0,0,0.92) 0%,rgba(0,0,0,0.15) 100%)',display:'flex',alignItems:'center',padding:'22px',gap:'14px'}}>
                    <div style={{flex:1}}><div style={{fontSize:'8px',color:'rgba(255,255,255,0.35)',letterSpacing:'2px',marginBottom:'8px'}}>{tx.ad} · TOYOTA ETHIOPIA</div><div style={{fontSize:'26px',fontWeight:900,color:'white',marginBottom:'4px',lineHeight:1}}>Land Cruiser</div><div style={{fontSize:'14px',fontWeight:700,color:'#60A5FA',marginBottom:'6px'}}>2025 Edition</div><div style={{fontSize:'11px',color:'rgba(255,255,255,0.65)'}}>The ultimate off-roader</div></div>
                ...
                {i===8 && <div key="ad3" style={{gridColumn:'1/-1',background:'linear-gradient(135deg,#5C0000,#8B0000)',borderRadius:'18px',padding:'18px',display:'flex',alignItems:'center',gap:'14px',cursor:'pointer',boxShadow:'0 8px 28px rgba(92,0,0,0.4)'}}>
                  ...
                  <div style={{background:'#F8FAFF',color:'#5C0000',fontSize:'12px',fontWeight:900,padding:'12px 16px',borderRadius:'14px',whiteSpace:'nowrap',flexShrink:0}}>Learn More</div>
                </div>}
                <div key={l.id} style={{background:'#F8FAFF',borderRadius:'18px',overflow:'hidden',boxShadow:'0 3px 14px rgba(0,0,0,0.08)',cursor:'pointer'}}>
                  ...
                  <div style={{padding:'14px 16px',display:'flex',alignItems:'center',justifyContent:'space-between'}}>
                    ...
                    <button style={{background:'#60A5FA',color:'white',border:'none',borderRadius:'12px',padding:'9px 22px',fontSize:'13px',fontWeight:800,cursor:'pointer'}} onClick={()=>window.location.href=`/listing/${l.id}`}>{tx.view}</button>
                  </div>
                </div>
              </>
            ))}
      ...
      <div style={{margin:'0 16px 20px',borderRadius:'18px',overflow:'hidden',cursor:'pointer',boxShadow:'0 8px 28px rgba(0,0,0,0.2)',position:'relative'}}>
        ...
        <div style={{position:'absolute',inset:0,background:'linear-gradient(135deg,rgba(0,100,0,0.93) 0%,rgba(0,60,0,0.55) 100%)',display:'flex',alignItems:'center',padding:'20px',gap:'16px'}}>
          ...
          <div style={{background:'#60A5FA',color:'#111',fontSize:'12px',fontWeight:900,padding:'12px 18px',borderRadius:'14px',whiteSpace:'nowrap',flexShrink:0}}>Book Now</div>
        </div>
      </div>
      ...
      <section style={{margin:'0 16px 24px',background:'linear-gradient(135deg,#090909,#191919)',borderRadius:'22px',padding:'30px 22px',textAlign:'center'}}>
        ...
        <div style={{display:'flex',gap:'12px',justifyContent:'center'}}>
          <button style={{background:'#F8FAFF',color:'#111',border:'none',borderRadius:'14px',padding:'14px 24px',fontSize:'14px',fontWeight:800,cursor:'pointer'}}>🍎 App Store</button>
          <button style={{background:'#60A5FA',color:'white',border:'none',borderRadius:'14px',padding:'14px 24px',fontSize:'14px',fontWeight:800,cursor:'pointer'}}>▶ Google Play</button>
        </div>
      </section>
      ...
      <footer style={{background:'#090909',padding:'32px 16px 28px'}}>
        <div style={{display:'flex',alignItems:'center',gap:'12px',marginBottom:'24px',paddingBottom:'20px',borderBottom:'1px solid rgba(255,255,255,0.06)'}}>
          ...
          <div><div style={{fontSize:'18px',fontWeight:900,color:'white',letterSpacing:'1.5px'}}>HAGERHUB</div><div style={{fontSize:'10px',color:'rgba(255,255,255,0.25)',letterSpacing:'1px'}}>The Hub of the Homeland · ሃገር ሃብ</div></div>
        </div>
        <div className='listings-grid grid grid-cols-1 md:grid-cols-3 gap-6 max-w-7xl mx-auto px-6'>
          {[{title:'Company',links:['About Us','Careers','Advertise with Us','Legal Hub']},{title:'Ethiopia',links:['Addis Ababa','Hawassa','Bahir Dar','Dire Dawa']},{title:'Support',links:['Help Center','Contact Us','Safety Tips','Report Listing']},{title:'Follow Us',links:['Facebook','Telegram','Instagram','TikTok']}].map(col=>(
            <div key={col.title}>
        ...
        <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',borderTop:'1px solid rgba(255,255,255,0.05)',paddingTop:'18px'}}>
          ...
          <div style={{display:'flex',height:'3px',width:'42px',borderRadius:'2px',overflow:'hidden'}}><div style={{flex:1,background:'#60A5FA'}}/><div style={{flex:1,background:'#60A5FA'}}/><div style={{flex:1,background:'#EF2118'}}/></div>
        </div>
      </footer>
    </main>
  )
