...
const PartnerBanner = ({bg,name,sub,cta,tag,accent='rgba(255,255,255,0.22)'}:{bg:string,name:string,sub:string,cta:string,tag:string,accent?:string}) => (
  <div style={{margin:'4px 0 28px'}}>
    <div className="partner-banner" style={{
      ...
      <div style={{position:'absolute',bottom:0,left:0,right:0,height:'55%',background:'linear-gradient(to top, rgba(0,0,0,0.28) 0%, rgba(0,0,0,0) 100%)',pointerEvents:'none'}}/><div style={{position:'absolute',top:'14px',right:'16px',zIndex:2,fontSize:'10px',fontWeight:800,letterSpacing:'1.5px',textTransform:'uppercase',color:'#fff',background:'rgba(255,255,255,0.18)',border:'1px solid rgba(255,255,255,0.45)',borderRadius:'999px',padding:'5px 12px',backdropFilter:'blur(4px)'}}>{tag}</div>
...
async function fetchListings() {
    setLoading(true)
    const supabase = createClient()
    let q = supabase.from('listings').select('*').ilike('status','active').order('created_at',{ascending:false})
...
@keyframes featuredshimmer { 0%,100%{box-shadow:0 0 0 rgba(218,165,32,0)} 50%{box-shadow:0 4px 20px rgba(218,165,32,0.15)} }
      ...
      <div id="results" style={{maxWidth:'1280px',margin:'0 auto',padding:'24px 20px',display:'grid',gridTemplateColumns: activeCat==='All' ? '1fr' : '200px 1fr',gap:'24px',alignItems:'start',scrollMarginTop:'70px'}}>
        ...
        <div style={{minWidth:0}}>
          ...
          {loading ? (
            ...
            (()=>{const pool=Object.values(AD_BANNERS);let bi=0;const secs=POPULAR.map(cat=>{
              const items=filtered.filter(l=>(l.category||'').trim().toLowerCase()===cat.name.trim().toLowerCase())
              if(items.length===0) return null
              const banner=pool[bi]!==undefined?pool[bi++]:null
              return (
                <div key={cat.key}>
                  {items.length>0 && (
      ...
      <div className="feat-band" style={{maxWidth:'1280px',margin:'0 auto',padding:'0 20px 8px',display:'grid',gridTemplateColumns:'repeat(auto-fit,minmax(240px,1fr))',gap:'14px',alignItems:'start'}}>
          ...
          {activeCat==='All' && (()=>{const pool=Object.values(AD_BANNERS);const n=POPULAR.filter(cat=>filtered.some(l=>(l.category||'').trim().toLowerCase()===cat.name.trim().toLowerCase())).length;return pool.slice(n).map((b,i)=>(<div key={'lo-'+i} style={{margin:'28px 0'}}>{b}</div>))})()}
