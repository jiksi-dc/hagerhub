...
const PUBLIC_COLS = 'id,title,description,price,price_label,category,subcategory,city,neighbourhood,status,views,created_at,user_id,image_urls,price_amount,price_currency,is_featured,is_top,boost_expires_at,admission_fee,event_date,event_end_date,event_time,venue,website,organizer,region,area_sqm,body_condition,body_type,employment_type,fuel_type,mechanical_condition,mileage,seller_type,bedrooms,bathrooms,make,model,year,transmission,purpose,company,salary,condition,capacity,address,amenities'
const CONTACT_COLS = PUBLIC_COLS + ',contact_phone,contact_name'

const HouseAdBanner = ({ locale }: { locale: string }) => (
  <a href={`/${locale}/advertise`} className="house-ad" style={{display:'block',position:'relative',width:'100%',aspectRatio:'2.94 / 1',borderRadius:'18px',overflow:'hidden',boxShadow:'0 6px 20px rgba(0,0,0,0.12)',textDecoration:'none'}}>
    <div className="house-ad-img" style={{position:'absolute',inset:0,backgroundImage:'url(/brand/advertise-card.jpg)',backgroundSize:'cover',backgroundPosition:'center'}}/>
    <div className="house-ad-sheen" style={{position:'absolute',inset:0,pointerEvents:'none'}}/>
  </a>
)

const AD_BANNERS: Record<string, (locale: string) => React.ReactNode> = {
  properties: (locale) => <HouseAdBanner locale={locale}/>,
  vehicles:   (locale) => <HouseAdBanner locale={locale}/>,
  machinery:  (locale) => <HouseAdBanner locale={locale}/>,
}
...
function EthiofyBrandLogo() {
  return (
    <span className="nav-logo">
      <Image
        src="/brand/ethiofy-lion-header-logo.png"
        alt="Ethiofy"
        width={1376}
        height={768}
...
export default function Home() {
  ...
  useEffect(()=>{
    ...
    if (v) { v.muted = true; v.play().then(()=>setAdPaused(false)).catch(()=>setAdPaused(true)) }
    const id = setInterval(()=>{
      const vv = adRef.current
      if (vv && vv.paused && vv.muted) { vv.play().then(()=>setAdPaused(false)).catch(()=>{}) }
    }, 3000)
...
const supabase = createClient()
const {data} = await supabase.from('listings').select(PUBLIC_COLS as any).eq('status','active').eq('is_featured',true).gte('boost_expires_at',new Date().toISOString()).order('created_at',{ascending:false}).limit(6)
setFeaturedListings((data as any)||[])
}
...
async function fetchListings() {
    ...
    let q = supabase.from('listings').select((u ? CONTACT_COLS : PUBLIC_COLS) as any).ilike('status','active').order('created_at',{ascending:false})
    if (activeCat==='Discover') q = q.ilike('category','Discover Ethiopia')
else if (activeCat!=='All') q = q.ilike('category',activeCat)
        ...
        <div style={{padding:'10px 12px 12px'}}>
          ...
          {(() => {
            ...
            if(!phone) return user ? null : (
              <a href={`/${locale}/login?next=/${locale}/listing/${l.id}`} onClick={(e)=>e.stopPropagation()}
                style={{display:'flex',alignItems:'center',justifyContent:'center',gap:'6px',width:'100%',background:'#fff',color:'#111',fontSize:'12px',fontWeight:700,padding:'8px',borderRadius:'8px',textDecoration:'none',fontFamily:'inherit',border:'1px solid #E5E7EB'}}>
                📞 Sign in to call
              </a>
            )
            return (
  ...
  const FilterPanel = () => (
    <div style={{position:'sticky',top:'120px',alignSelf:'start',background:'#fff',borderRadius:'14px',border:'1px solid #F3F4F6',padding:'18px',display:'flex',flexDirection:'column',gap:'18px',minWidth:'200px'}}>
      ...
      {activeCat==='Properties' && (<>
        <div><label style={filterLabel}>Purpose</label>
          <select value={filterPurpose} onChange={e=>setFilterPurpose(e.target.value)} style={filterSelect}>
            <option value="">Any</option>{HERO_PURPOSE.map(o=><option key={o} value={o}>{o}</option>)}
          </select></div>
        <div><label style={filterLabel}>Furnishing</label>
          <select value={filterFurnished} onChange={e=>setFilterFurnished(e.target.value)} style={filterSelect}>
            <option value="">Any</option><option value="Furnished">Furnished</option><option value="Unfurnished">Unfurnished</option>
          </select></div>
        <div><label style={filterLabel}>Bedrooms</label>
          <select value={filterBeds} onChange={e=>setFilterBeds(e.target.value)} style={filterSelect}>
            <option value="">Any</option>{['1','2','3','4','5'].map(o=><option key={o} value={o}>{o}+</option>)}
          </select></div>
        <div><label style={filterLabel}>Bathrooms</label>
          <select value={filterBaths} onChange={e=>setFilterBaths(e.target.value)} style={filterSelect}>
            <option value="">Any</option>{['1','2','3','4'].map(o=><option key={o} value={o}>{o}+</option>)}
          </select></div>
      </>)}
      {activeCat==='Vehicles' && (<>
        <div><label style={filterLabel}>Make</label>
          <select value={filterMake} onChange={e=>setFilterMake(e.target.value)} style={filterSelect}>
            <option value="">All makes</option>{HERO_MAKES.map(o=><option key={o} value={o}>{o}</option>)}
          </select></div>
        <div><label style={filterLabel}>Fuel</label>
          <select value={filterFuel} onChange={e=>setFilterFuel(e.target.value)} style={filterSelect}>
            <option value="">Any</option>{HERO_FUEL.map(o=><option key={o} value={o}>{o}</option>)}
          </select></div>
        <div><label style={filterLabel}>Transmission</label>
          <select value={filterTransmission} onChange={e=>setFilterTransmission(e.target.value)} style={filterSelect}>
            <option value="">Any</option>{HERO_TRANSMISSION.map(o=><option key={o} value={o}>{o}</option>)}
          </select></div>
        <div><label style={filterLabel}>Min year</label>
          <select value={filterMinYear} onChange={e=>setFilterMinYear(e.target.value)} style={filterSelect}>
            <option value="">Any</option>{HERO_YEARS.map(o=><option key={o} value={o}>{o}</option>)}
          </select></div>
      </>)}
      {(activeCat==='Machinery' || activeCat==='Classifieds') && (
        <div><label style={filterLabel}>Condition</label>
      ...
      {activeCat==='Jobs' && (<>
        <div><label style={filterLabel}>Field</label>
          <select value={filterField} onChange={e=>setFilterField(e.target.value)} style={filterSelect}>
            <option value="">All fields</option>{HERO_FIELDS.map(o=><option key={o} value={o}>{o}</option>)}
          </select></div>
        <div><label style={filterLabel}>Employment</label>
          <select value={filterEmployment} onChange={e=>setFilterEmployment(e.target.value)} style={filterSelect}>
  ...
  return (
    <main style={{minHeight:'100vh',background:'#F9FAFB',fontFamily:'-apple-system,BlinkMacSystemFont,"Segoe UI",sans-serif'}}>
      <style>{`
        @keyframes houseAdDrift{0%{transform:scale(1.04) translateX(0)}50%{transform:scale(1.08) translateX(-1.2%)}100%{transform:scale(1.04) translateX(0)}}
        ...
        .house-ad{transition:transform .25s ease,box-shadow .25s ease}
        ...
        nav{overflow:hidden}
        nav::before{content:'';position:absolute;left:0;top:0;width:96px;height:96px;background-image:url("data:image/svg+xml;utf8,%3Csvg%20xmlns%3D%22http%3A%2F%2Fwww.w3.org%2F2000%2Fsvg%22%20viewBox%3D%220%200%2096%2096%22%20width%3D%2296%22%20height%3D%2296%22%3E%3Cpath%20d%3D%22M0.0%200%20Q%2048.0%2048.0%2C%200%200.0%22%20stroke%3D%22%23307830%22%20stroke-width%3D%220.7%22%20fill%3D%22none%22%20opacity%3D%220.55%22%2F%3E%3Cpath%20d%3D%22M3.7%200%20Q%2048.0%2046.2%2C%200%203.7%22%20stroke%3D%22%23D8C048%22%20stroke-width%3D%220.7%22%20fill%3D%22none%22%20opacity%3D%220.55%22%2F%3E%3Cpath%20d%3D%22M7.4%200%20Q%2048.0%2044.3%2C%200%207.4%22%20stroke%3D%22%23904830%22%20stroke-width%3D%220.7%22%20fill%3D%22none%22%20opacity%3D%220.55%22%2F%3E%3Cpath%20d%3D%22M11.1%200%20Q%2048.0%2042.5%2C%200%2011.1%22%20stroke%3D%22%23307830%22%20stroke-width%3D%220.7%22%20fill%3D%22none%22%20opacity%3D%220.55%22%2F%3E%3Cpath%20d%3D%22M14.8%200%20Q%2048.0%2040.6%2C%200%2014.8%22%20stroke%3D%22%23D8C048%22%20stroke-width%3D%220.7%22%20fill%3D%22none%22%20opacity%3D%220.55%22%2F%3E%3Cpath%20d%3D%22M18.5%200%20Q%2048.0%2038.8%2C%200%2018.5%22%20stroke%3D%22%23904830%22%20stroke-width%3D%220.7%22%20fill%3D%22none%22%20opacity%3D%220.55%22%2F%3E%3Cpath%20d%3D%22M22.2%200%20Q%2048.0%2036.9%2C%200%2022.2%22%20stroke%3D%22%23307830%22%20stroke-width%3D%220.7%22%20fill%3D%22none%22%20opacity%3D%220.55%22%2F%3E%3Cpath%20d%3D%22M25.8%200%20Q%2048.0%2035.1%2C%200%2025.8%22%20stroke%3D%22%23D8C048%22%20stroke-width%3D%220.7%22%20fill%3D%22none%22%20opacity%3D%220.55%22%2F%3E%3Cpath%20d%3D%22M29.5%200%20Q%2048.0%2033.2%2C%200%2029.5%22%20stroke%3D%22%23904830%22%20stroke-width%3D%220.7%22%20fill%3D%22none%22%20opacity%3D%220.55%22%2F%3E%3Cpath%20d%3D%22M33.2%200%20Q%2048.0%2031.4%2C%200%2033.2%22%20stroke%3D%22%23307830%22%20stroke-width%3D%220.7%22%20fill%3D%22none%22%20opacity%3D%220.55%22%2F%3E%3Cpath%20d%3D%22M36.9%200%20Q%2048.0%2029.5%2C%200%2036.9%22%20stroke%3D%22%23D8C048%22%20stroke-width%3D%220.7%22%20fill%3D%22none%22%20opacity%3D%220.55%22%2F%3E%3Cpath%20d%3D%22M40.6%200%20Q%2048.0%2027.7%2C%200%2040.6%22%20stroke%3D%22%23904830%22%20stroke-width%3D%220.7%22%20fill%3D%22none%22%20opacity%3D%220.55%22%2F%3E%3Cpath%20d%3D%22M44.3%200%20Q%2048.0%2025.8%2C%200%2044.3%22%20stroke%3D%22%23307830%22%20stroke-width%3D%220.7%22%20fill%3D%22none%22%20opacity%3D%220.55%22%2F%3E%3Cpath%20d%3D%22M48.0%200%20Q%2048.0%2024.0%2C%200%2048.0%22%20stroke%3D%22%23D8C048%22%20stroke-width%3D%220.7%22%20fill%3D%22none%22%20opacity%3D%220.55%22%2F%3E%3Cpath%20d%3D%22M51.7%200%20Q%2048.0%2022.2%2C%200%2051.7%22%20stroke%3D%22%23904830%22%20stroke-width%3D%220.7%22%20fill%3D%22none%22%20opacity%3D%220.55%22%2F%3E%3Cpath%20d%3D%22M55.4%200%20Q%2048.0%2020.3%2C%200%2055.4%22%20stroke%3D%22%23307830%22%20stroke-width%3D%220.7%22%20fill%3D%22none%22%20opacity%3D%220.55%22%2F%3E%3Cpath%20d%3D%22M59.1%200%20Q%2048.0%2018.5%2C%200%2059.1%22%20stroke%3D%22%23D8C048%22%20stroke-width%3D%220.7%22%20fill%3D%22none%22%20opacity%3D%220.55%22%2F%3E%3Cpath%20d%3D%22M62.8%200%20Q%2048.0%2016.6%2C%200%2062.8%22%20stroke%3D%22%23904830%22%20stroke-width%3D%220.7%22%20fill%3D%22none%22%20opacity%3D%220.55%22%2F%3E%3Cpath%20d%3D%22M66.5%200%20Q%2048.0%2014.8%2C%200%2066.5%22%20stroke%3D%22%23307830%22%20stroke-width%3D%220.7%22%20fill%3D%22none%22%20opacity%3D%220.55%22%2F%3E%3Cpath%20d%3D%22M70.2%200%20Q%2048.0%2012.9%2C%200%2070.2%22%20stroke%3D%22%23D8C048%22%20stroke-width%3D%220.7%22%20fill%3D%22none%22%20opacity%3D%220.55%22%2F%3E%3Cpath%20d%3D%22M73.8%200%20Q%2048.0%2011.1%2C%200%2073.8%22%20stroke%3D%22%23904830%22%20stroke-width%3D%220.7%22%20fill%3D%22none%22%20opacity%3D%220.55%22%2F%3E%3Cpath%20d%3D%22M77.5%200%20Q%2048.0%209.2%2C%200%2077.5%22%20stroke%3D%22%23307830%22%20stroke-width%3D%220.7%22%20fill%3D%22none%22%20opacity%3D%220.55%22%2F%3E%3Cpath%20d%3D%22M81.2%200%20Q%2048.0%207.4%2C%200%2081.2%22%20stroke%3D%22%23D8C048%22%20stroke-width%3D%220.7%22%20fill%3D%22none%22%20opacity%3D%220.55%22%2F%3E%3Cpath%20d%3D%22M84.9%200%20Q%2048.0%205.5%2C%200%2084.9%22%20stroke%3D%22%23904830%22%20stroke-width%3D%220.7%22%20fill%3D%22none%22%20opacity%3D%220.55%22%2F%3E%3Cpath%20d%3D%22M88.6%200%20Q%2048.0%203.7%2C%200%2088.6%22%20stroke%3D%22%23307830%22%20stroke-width%3D%220.7%22%20fill%3D%22none%22%20opacity%3D%220.55%22%2F%3E%3Cpath%20d%3D%22M92.3%200%20Q%2048.0%201.8%2C%200%2092.3%22%20stroke%3D%22%23D8C048%22%20stroke-width%3D%220.7%22%20fill%3D%22none%22%20opacity%3D%220.55%22%2F%3E%3C%2Fsvg%3E");background-size:cover;pointer-events:none;z-index:0}
        nav::after{content:'';position:absolute;right:0;top:0;width:96px;height:96px;background-image:url("data:image/svg+xml;utf8,%3Csvg%20xmlns%3D%22http%3A%2F%2Fwww.w3.org%2F2000%2Fsvg%22%20viewBox%3D%220%200%2096%2096%22%20width%3D%2296%22%20height%3D%2296%22%3E%3Cpath%20d%3D%22M0.0%200%20Q%2048.0%2048.0%2C%200%200.0%22%20stroke%3D%22%23307830%22%20stroke-width%3D%220.7%22%20fill%3D%22none%22%20opacity%3D%220.55%22%2F%3E%3Cpath%20d%3D%22M3.7%200%20Q%2048.0%2046.2%2C%200%203.7%22%20stroke%3D%22%23D8C048%22%20stroke-width%3D%220.7%22%20fill%3D%22none%22%20opacity%3D%220.55%22%2F%3E%3Cpath%20d%3D%22M7.4%200%20Q%2048.0%2044.3%2C%200%207.4%22%20stroke%3D%22%23904830%22%20stroke-width%3D%220.7%22%20fill%3D%22none%22%20opacity%3D%220.55%22%2F%3E%3Cpath%20d%3D%22M11.1%200%20Q%2048.0%2042.5%2C%200%2011.1%22%20stroke%3D%22%23307830%22%20stroke-width%3D%220.7%22%20fill%3D%22none%22%20opacity%3D%220.55%22%2F%3E%3Cpath%20d%3D%22M14.8%200%20Q%2048.0%2040.6%2C%200%2014.8%22%20stroke%3D%22%23D8C048%22%20stroke-width%3D%220.7%22%20fill%3D%22none%22%20opacity%3D%220.55%22%2F%3E%3Cpath%20d%3D%22M18.5%200%20Q%2048.0%2038.8%2C%200%2018.5%22%20stroke%3D%22%23904830%22%20stroke-width%3D%220.7%22%20fill%3D%22none%22%20opacity%3D%220.55%22%2F%3E%3Cpath%20d%3D%22M22.2%200%20Q%2048.0%2036.9%2C%200%2022.2%22%20stroke%3D%22%23307830%22%20stroke-width%3D%220.7%22%20fill%3D%22none%22%20opacity%3D%220.55%22%2F%3E%3Cpath%20d%3D%22M25.8%200%20Q%2048.0%2035.1%2C%200%2025.8%22%20stroke%3D%22%23D8C048%22%20stroke-width%3D%220.7%22%20fill%3D%22none%22%20opacity%3D%220.55%22%2F%3E%3Cpath%20d%3D%22M29.5%200%20Q%2048.0%2033.2%2C%200%2029.5%22%20stroke%3D%22%23904830%22%20stroke-width%3D%220.7%22%20fill%3D%22none%22%20opacity%3D%220.55%22%2F%3E%3Cpath%20d%3D%22M33.2%200%20Q%2048.0%2031.4%2C%200%2033.2%22%20stroke%3D%22%23307830%22%20stroke-width%3D%220.7%22%20fill%3D%22none%22%20opacity%3D%220.55%22%2F%3E%3Cpath%20d%3D%22M36.9%200%20Q%2048.0%2029.5%2C%200%2036.9%22%20stroke%3D%22%23D8C048%22%20stroke-width%3D%220.7%22%20fill%3D%22none%22%20opacity%3D%220.55%22%2F%3E%3Cpath%20d%3D%22M40.6%200%20Q%2048.0%2027.7%2C%200%2040.6%22%20stroke%3D%22%23904830%22%20stroke-width%3D%220.7%22%20fill%3D%22none%22%20opacity%3D%220.55%22%2F%3E%3Cpath%20d%3D%22M44.3%200%20Q%2048.0%2025.8%2C%200%2044.3%22%20stroke%3D%22%23307830%22%20stroke-width%3D%220.7%22%20fill%3D%22none%22%20opacity%3D%220.55%22%2F%3E%3Cpath%20d%3D%22M48.0%200%20Q%2048.0%2024.0%2C%200%2048.0%22%20stroke%3D%22%23D8C048%22%20stroke-width%3D%220.7%22%20fill%3D%22none%22%20opacity%3D%220.55%22%2F%3E%3Cpath%20d%3D%22M51.7%200%20Q%2048.0%2022.2%2C%200%2051.7%22%20stroke%3D%22%23904830%22%20stroke-width%3D%220.7%22%20fill%3D%22none%22%20opacity%3D%220.55%22%2F%3E%3Cpath%20d%3D%22M55.4%200%20Q%2048.0%2020.3%2C%200%2055.4%22%20stroke%3D%22%23307830%22%20stroke-width%3D%220.7%22%20fill%3D%22none%22%20opacity%3D%220.55%22%2F%3E%3Cpath%20d%3D%22M59.1%200%20Q%2048.0%2018.5%2C%200%2059.1%22%20stroke%3D%22%23D8C048%22%20stroke-width%3D%220.7%22%20fill%3D%22none%22%20opacity%3D%220.55%22%2F%3E%3Cpath%20d%3D%22M62.8%200%20Q%2048.0%2016.6%2C%200%2062.8%22%20stroke%3D%22%23904830%22%20stroke-width%3D%220.7%22%20fill%3D%22none%22%20opacity%3D%220.55%22%2F%3E%3Cpath%20d%3D%22M66.5%200%20Q%2048.0%2014.8%2C%200%2066.5%22%20stroke%3D%22%23307830%22%20stroke-width%3D%220.7%22%20fill%3D%22none%22%20opacity%3D%220.55%22%2F%3E%3Cpath%20d%3D%22M70.2%200%20Q%2048.0%2012.9%2C%200%2070.2%22%20stroke%3D%22%23D8C048%22%20stroke-width%3D%220.7%22%20fill%3D%22none%22%20opacity%3D%220.55%22%2F%3E%3Cpath%20d%3D%22M73.8%200%20Q%2048.0%2011.1%2C%200%2073.8%22%20stroke%3D%22%23904830%22%20stroke-width%3D%220.7%22%20fill%3D%22none%22%20opacity%3D%220.55%22%2F%3E%3Cpath%20d%3D%22M77.5%200%20Q%2048.0%209.2%2C%200%2077.5%22%20stroke%3D%22%23307830%22%20stroke-width%3D%220.7%22%20fill%3D%22none%22%20opacity%3D%220.55%22%2F%3E%3Cpath%20d%3D%22M81.2%200%20Q%2048.0%207.4%2C%200%2081.2%22%20stroke%3D%22%23D8C048%22%20stroke-width%3D%220.7%22%20fill%3D%22none%22%20opacity%3D%220.55%22%2F%3E%3Cpath%20d%3D%22M84.9%200%20Q%2048.0%205.5%2C%200%2084.9%22%20stroke%3D%22%23904830%22%20stroke-width%3D%220.7%22%20fill%3D%22none%22%20opacity%3D%220.55%22%2F%3E%3Cpath%20d%3D%22M88.6%200%20Q%2048.0%203.7%2C%200%2088.6%22%20stroke%3D%22%23307830%22%20stroke-width%3D%220.7%22%20fill%3D%22none%22%20opacity%3D%220.55%22%2F%3E%3Cpath%20d%3D%22M92.3%200%20Q%2048.0%201.8%2C%200%2092.3%22%20stroke%3D%22%23D8C048%22%20stroke-width%3D%220.7%22%20fill%3D%22none%22%20opacity%3D%220.55%22%2F%3E%3C%2Fsvg%3E");background-size:cover;transform:scaleX(-1);pointer-events:none;z-index:0}
        .nav-inner{position:relative;z-index:1}
        .nav-taglines{position:relative;padding-bottom:14px}
        .nav-taglines::after{content:'';position:absolute;left:0;right:0;bottom:0;height:12px;background-image:url("data:image/svg+xml;utf8,%3Csvg%20xmlns%3D%22http%3A%2F%2Fwww.w3.org%2F2000%2Fsvg%22%20viewBox%3D%220%200%20360%2012%22%20width%3D%22360%22%20height%3D%2212%22%3E%3Cpath%20d%3D%22M0%203.6%20C%2090.0%200.6%2C%20216.0%207.2%2C%20360%202.4%22%20stroke%3D%22%23307830%22%20stroke-width%3D%221.6%22%20fill%3D%22none%22%20opacity%3D%220.9%22%20stroke-linecap%3D%22round%22%2F%3E%3Cpath%20d%3D%22M0%206.0%20C%2090.0%203.0%2C%20216.0%209.6%2C%20360%204.8%22%20stroke%3D%22%23D8C048%22%20stroke-width%3D%221.6%22%20fill%3D%22none%22%20opacity%3D%220.9%22%20stroke-linecap%3D%22round%22%2F%3E%3Cpath%20d%3D%22M0%208.4%20C%2090.0%205.4%2C%20216.0%2012.0%2C%20360%207.2%22%20stroke%3D%22%23904830%22%20stroke-width%3D%221.6%22%20fill%3D%22none%22%20opacity%3D%220.9%22%20stroke-linecap%3D%22round%22%2F%3E%3C%2Fsvg%3E");background-size:100% 12px;background-repeat:no-repeat;pointer-events:none}
        .popular-h{position:relative;padding-bottom:15px}
        .popular-h::after{content:'';position:absolute;left:0;right:0;bottom:0;height:12px;background-image:url("data:image/svg+xml;utf8,%3Csvg%20xmlns%3D%22http%3A%2F%2Fwww.w3.org%2F2000%2Fsvg%22%20viewBox%3D%220%200%20300%2012%22%20width%3D%22300%22%20height%3D%2212%22%3E%3Cpath%20d%3D%22M0%203.4%20C%2075.0%200.7%2C%20180.0%206.7%2C%20300%202.2%22%20stroke%3D%22%231F8A3B%22%20stroke-width%3D%221.7%22%20fill%3D%22none%22%20opacity%3D%220.9%22%20stroke-linecap%3D%22round%22%2F%3E%3Cpath%20d%3D%22M0%206.0%20C%2075.0%203.4%2C%20180.0%209.4%2C%20300%204.8%22%20stroke%3D%22%23F2C500%22%20stroke-width%3D%221.7%22%20fill%3D%22none%22%20opacity%3D%220.9%22%20stroke-linecap%3D%22round%22%2F%3E%3Cpath%20d%3D%22M0%208.6%20C%2075.0%206.0%2C%20180.0%2012.0%2C%20300%207.4%22%20stroke%3D%22%23D22B1E%22%20stroke-width%3D%221.7%22%20fill%3D%22none%22%20opacity%3D%220.9%22%20stroke-linecap%3D%22round%22%2F%3E%3C%2Fsvg%3E");background-size:100% 12px;background-repeat:no-repeat;pointer-events:none}
...
@keyframes featuredshimmer { 0%,100%{box-shadow:0 0 0 rgba(218,165,32,0)} 50%{box-shadow:0 4px 20px rgba(218,165,32,0.15)} }
        *{box-sizing:border-box;margin:0;padding:0}
        ...
        @media (max-width: 768px){.hero-tabs::-webkit-scrollbar{display:none}} .hero-tabs{scrollbar-width:thin}
        ...
        @media (max-width: 768px) {
          ...
          .hero-section { min-height: 0 !important; background: #f9fafb !important; }
          ...
          .hero-inner { padding: 12px 16px 4px !important; gap: 10px !important; flex-direction: column !important; align-items: stretch !important; }          .hero-inner > div:not(.hero-video) { max-width: 100% !important; width: 100% !important; flex: none !important; }          .hero-video { order: -1 !important; width: 100% !important; height: auto !important; aspect-ratio: 3/2 !important; flex: none !important; }
          .hero-panel { width: 100% !important; padding: 14px !important; margin-top: 0 !important; } .hero-mobilebtns { display: none !important; } .hero-panel { display: none !important; } .hero-panel.mob-open { display: block !important; } .hero-mobile-ai { display: flex !important; } .hero-mobilebtns button { white-space: nowrap !important; font-size: 15px !important; height: 52px !important; padding: 0 22px !important; gap: 7px !important; border-radius: 12px !important; flex: 1 1 0 !important; min-width: 0 !important; max-width: 200px !important; box-sizing: border-box !important; background: #2563EB !important; color: #fff !important; border-color: #2563EB !important; } .browse-box { display: none !important; }
          .hero-tabs { display: none !important; }
          .hero-title { display: none !important; }
          .popular-h { font-size: 22px !important; font-weight: 900 !important; color: #111 !important; }
          #results { padding-top: 2px !important; }
          .cats-wrap { display: block !important; position: relative; width: 100%; margin: 12px 0 14px; }
          .cats-wrap::after { display: none; }
          .cats-wrap::before { display: none; }
          .nav-cats { display: grid; grid-template-columns: 1fr 1fr 1fr; width: 100%; gap: 7px; padding: 0 10px 8px 10px; }
          .nav-cats::-webkit-scrollbar { height: 3px; }
          .nav-cats::-webkit-scrollbar-thumb { background: #D1D5DB; border-radius: 3px; }
          .nav-cats button { font-size: 13px !important; font-weight: 700 !important; padding: 9px 6px !important; white-space: nowrap; text-align: center; }
            ...
            {aiOpen && (
              <div style={{ position:'absolute', top:'calc(100% + 6px)', left:0, right:0, background:'#fff', border:'1px solid #C7C4F5', borderRadius:'12px', boxShadow:'0 10px 32px rgba(0,0,0,0.14)', zIndex:9999, display:'flex', flexDirection:'column', overflow:'hidden' }}>
                <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', padding:'10px 14px', borderBottom:'1px solid #eee', background:'#F7F7FB' }}>
                  <span style={{ display:'inline-flex', alignItems:'center', gap:'7px', fontSize:'13px', fontWeight:600, color:'#111' }}><span style={{ color:'#4F46E5', fontSize:'15px' }}>✦</span> Ethiofy AI</span>
      ...
      <section className="hero-section" style={{position:'relative',width:'100%',minHeight:'340px',background:'#f9fafb',overflow:'hidden'}}>
        ...
        <div className="hero-video" style={{flex:'0 0 auto',width:'640px',height:'360px',borderRadius:'12px',overflow:'hidden',position:'relative',background:'#000'}}>
          <video ref={adRef} src="/ethiofy_ad_video_6.mp4" autoPlay muted={adMuted} loop playsInline onPlay={()=>setAdPaused(false)} onPause={()=>{const v=adRef.current; if(v && v.muted){ v.play().then(()=>setAdPaused(false)).catch(()=>setAdPaused(true)) } else { setAdPaused(true) }}} style={{width:'100%',height:'100%',objectFit:'cover',display:'block'}} />{adPaused && <button onClick={()=>{adRef.current?.play()}} aria-label="Play ad" style={{position:'absolute',top:'50%',left:'50%',transform:'translate(-50%,-50%)',width:'64px',height:'64px',borderRadius:'50%',border:'none',background:'rgba(0,0,0,0.6)',color:'#fff',fontSize:'26px',cursor:'pointer',display:'flex',alignItems:'center',justifyContent:'center'}}>{'\u25B6'}</button>}<button onClick={()=>setAdMuted(m=>!m)} aria-label={adMuted?'Unmute ad':'Mute ad'} style={{position:'absolute',bottom:'12px',right:'12px',width:'38px',height:'38px',borderRadius:'50%',border:'none',background:'rgba(0,0,0,0.55)',color:'#fff',fontSize:'17px',cursor:'pointer',display:'flex',alignItems:'center',justifyContent:'center'}}>{adMuted?'\u{1F507}':'\u{1F50A}'}</button>
        </div>
        </div>
      </section>
      ...
      <div id="results" style={{maxWidth:'1280px',margin:'0 auto',padding:'24px 20px',display:'grid',gridTemplateColumns: activeCat==='All' ? '1fr' : '200px 1fr',gap:'24px',alignItems:'start',scrollMarginTop:'70px'}}>
        ...
        <div style={{minWidth:0}}>
          ...
          {loading ? (
            ...
            POPULAR.map(cat=>{
              const items=filtered.filter(l=>(l.category||'').trim().toLowerCase()===cat.name.trim().toLowerCase())
              const banner=AD_BANNERS[cat.key]?.(locale)
              return (
                <div key={cat.key}>
                  {(
                    <div style={{marginBottom:'24px'}}>
                      <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:'12px'}}>
                        <h2 className="popular-h" style={{fontSize:'15px',fontWeight:700,color:'#111',margin:0}}>
                  ...
                  {items.length===0 && <div style={{fontSize:'13px',color:'#9CA3AF',margin:'-14px 0 22px'}}>No listings yet — be the first to post.</div>}
                  {banner}
                </div>
              )
            })
      ...
      <footer style={{background:'#fff',borderTop:'1px solid #EBEBEB',padding:'32px 20px 24px',marginTop:'16px'}}>
        <div style={{maxWidth:'1280px',margin:'0 auto'}}>
          <div style={{fontSize:'15px',fontWeight:900,color:'#111',letterSpacing:'2px',marginBottom:'4px'}}>ETHIOFY</div>
          ...
          <div style={{borderTop:'1px solid #EBEBEB',paddingTop:'16px',display:'flex',justifyContent:'space-between',alignItems:'center',flexWrap:'wrap',gap:'8px'}}>
            <span style={{color:'#9CA3AF',fontSize:'12px'}}>© 2026 Ethiofy</span>
