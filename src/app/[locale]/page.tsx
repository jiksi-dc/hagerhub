    ...
    <main style={{fontFamily:'inherit',background:'#EEF2FF',minHeight:'100vh',width:'100%',overflowX:'hidden'}}>
      ...
      {activeCat==='All' && (
        <section style={{background:'#EEF2FF',padding:'22px 16px',borderBottom:'8px solid #F0F2F5'}}>
          ...
          <div style={{display:'grid',gridTemplateColumns:'repeat(2,1fr)',gap:'12px'}}>
            {POPULAR.map(cat=>(
              <div key={cat.name} onClick={()=>setActiveCat(cat.name)} style={{background:'#EEF2FF'}15,${cat.color}05)`,border:'1px solid #c7d2fe'}20`,borderRadius:'16px',padding:'16px 14px',cursor:'pointer'}}>
      ...
      <section style={{padding:'18px 16px'}}>
        <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:'18px'}}>
          ...
          {activeCat!=='All' && (<button onClick={()=>setActiveCat('All')} style={{fontSize:'12px',color:'#0057D9',background:'#EEF2FF',border:'1.5px solid #0057D9',borderRadius:'20px',padding:'8px 18px',cursor:'pointer',fontWeight:700}}>{tx.back}</button>)}
        </div>
