...
const POPULAR = [
  { name: 'Properties', items: ['Residential for Rent', 'Residential for Sale', 'Commercial', 'Land & Plots'], color: '#9CA3AF', emoji: '🏠' },
...
const CAT_COLORS: Record<string,string> = { Properties:'#9CA3AF', Vehicles:'#1B6BB5', Machinery:'#C9A84C', Classifieds:'#8B5E3C', Jobs:'#EF2118' }
    ...
    <main style={{fontFamily:'inherit',background:'#FFFFFF',minHeight:'100vh',width:'100%',overflowX:'hidden'}}>
      <section style={{background:'#FFFFFF',padding:'28px 16px 24px',textAlign:'center',position:'relative',overflow:'visible'}}>
        ...
        <div style={{display:'flex',background:'#FFFFFF',borderRadius:'14px',overflow:'hidden',boxShadow:'0 2px 12px rgba(0,0,0,0.1)',border:'1.5px solid #eee',marginBottom:'16px'}}>
          ...
          <button onClick={fetchListings} style={{background:'#9CA3AF',border:'none',color:'white',padding:'15px 22px',fontSize:'14px',fontWeight:800,cursor:'pointer',whiteSpace:'nowrap'}}>SEARCH</button>
        </div>
      </section>
      <div style={{background:"white",padding:"10px 16px",display:"flex",gap:"10px",alignItems:"center",borderBottom:"1px solid #eee"}}><AuthButton /><a href="/post" style={{background:"#9CA3AF",color:"white",padding:"8px 18px",borderRadius:"20px",fontSize:"13px",fontWeight:700,textDecoration:"none"}}>+ Post Ad</a></div>
      ...
      <div style={{display:'flex',overflowX:'auto',background:'#FFFFFF',boxShadow:'0 2px 10px rgba(0,0,0,0.07)',justifyContent:'center'}}>
        {CATS.map(c=>(<button key={c} onClick={()=>setActiveCat(c)} style={{padding:'15px 20px',fontSize:'13px',border:'none',flexShrink:0,borderBottom:activeCat===c?'3px solid #0057D9':'3px solid transparent',background:'none',color:activeCat===c?'#9CA3AF':'#777',fontWeight:activeCat===c?800:500,cursor:'pointer',whiteSpace:'nowrap'}}>{tl(c)}</button>))}
      </div>
      ...
      {activeCat==='All' && (
        <section style={{background:'#FFFFFF',padding:'22px 16px',borderBottom:'8px solid #F0F2F5'}}>
          <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:'18px'}}>
            ...
            <span style={{fontSize:'13px',color:'#9CA3AF',fontWeight:700,cursor:'pointer'}}>{tx.seeAll}</span>
          </div>
      ...
      <section style={{padding:'18px 16px'}}>
        <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:'18px'}}>
          ...
          {activeCat!=='All' && (<button onClick={()=>setActiveCat('All')} style={{fontSize:'12px',color:'#9CA3AF',background:'#FFFFFF',border:'1.5px solid #0057D9',borderRadius:'20px',padding:'8px 18px',cursor:'pointer',fontWeight:700}}>{tx.back}</button>)}
        </div>
            ...
            {filtered.map((l,i)=>(
              <>
                {i===2 && <div key="ad1" style={{gridColumn:'1/-1',background:'linear-gradient(135deg,#001A6E,#0041C4)',borderRadius:'18px',padding:'18px',display:'flex',alignItems:'center',gap:'14px',cursor:'pointer',boxShadow:'0 8px 28px rgba(0,26,110,0.4)'}}>
                  ...
                  <div style={{background:'#9CA3AF',color:'#001A6E',fontSize:'12px',fontWeight:900,padding:'12px 16px',borderRadius:'14px',whiteSpace:'nowrap',flexShrink:0}}>Apply Now</div>
                </div>}
                {i===5 && <div key="ad2" style={{gridColumn:'1/-1',borderRadius:'18px',overflow:'hidden',cursor:'pointer',boxShadow:'0 8px 28px rgba(0,0,0,0.2)',position:'relative'}}>
                  ...
                  <div style={{position:'absolute',inset:0,background:'linear-gradient(to right,rgba(0,0,0,0.92) 0%,rgba(0,0,0,0.15) 100%)',display:'flex',alignItems:'center',padding:'22px',gap:'14px'}}>
                    <div style={{flex:1}}><div style={{fontSize:'8px',color:'rgba(255,255,255,0.35)',letterSpacing:'2px',marginBottom:'8px'}}>{tx.ad} · TOYOTA ETHIOPIA</div><div style={{fontSize:'26px',fontWeight:900,color:'white',marginBottom:'4px',lineHeight:1}}>Land Cruiser</div><div style={{fontSize:'14px',fontWeight:700,color:'#9CA3AF',marginBottom:'6px'}}>2025 Edition</div><div style={{fontSize:'11px',color:'rgba(255,255,255,0.65)'}}>The ultimate off-roader</div></div>
                ...
                <div key={l.id} style={{background:'#FFFFFF',borderRadius:'18px',overflow:'hidden',boxShadow:'0 3px 14px rgba(0,0,0,0.08)',cursor:'pointer'}}>
                  <div style={{position:'relative',height:isDesktop?'420px':'280px',overflow:'hidden'}}>
                    ...
                    <div style={{position:'absolute',top:'12px',left:'12px',background:CAT_COLORS[l.category]||'#9CA3AF',color:'white',fontSize:'10px',fontWeight:800,padding:'5px 14px',borderRadius:'20px',letterSpacing:'0.5px'}}>{tl(l.category)}</div>
                  ...
                  <div style={{padding:'14px 16px',display:'flex',alignItems:'center',justifyContent:'space-between'}}>
                    ...
                    <button style={{background:'#9CA3AF',color:'white',border:'none',borderRadius:'12px',padding:'9px 22px',fontSize:'13px',fontWeight:800,cursor:'pointer'}} onClick={()=>window.location.href=`/listing/${l.id}`}>{tx.view}</button>
                  </div>
                </div>
              </>
            ))}
      ...
      <div style={{margin:'0 16px 20px',borderRadius:'18px',overflow:'hidden',cursor:'pointer',boxShadow:'0 8px 28px rgba(0,0,0,0.2)',position:'relative'}}>
        ...
        <div style={{position:'absolute',inset:0,background:'linear-gradient(135deg,rgba(0,100,0,0.93) 0%,rgba(0,60,0,0.55) 100%)',display:'flex',alignItems:'center',padding:'20px',gap:'16px'}}>
          ...
          <div style={{background:'#9CA3AF',color:'#111',fontSize:'12px',fontWeight:900,padding:'12px 18px',borderRadius:'14px',whiteSpace:'nowrap',flexShrink:0}}>Book Now</div>
        </div>
      </div>
      ...
      <section style={{margin:'0 16px 24px',background:'linear-gradient(135deg,#090909,#191919)',borderRadius:'22px',padding:'30px 22px',textAlign:'center'}}>
        ...
        <div style={{display:'flex',gap:'12px',justifyContent:'center'}}>
          ...
          <button style={{background:'#9CA3AF',color:'white',border:'none',borderRadius:'14px',padding:'14px 24px',fontSize:'14px',fontWeight:800,cursor:'pointer'}}>▶ Google Play</button>
        </div>
      </section>
      ...
      <footer style={{background:'#090909',padding:'32px 16px 28px'}}>
        ...
        <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',borderTop:'1px solid rgba(255,255,255,0.05)',paddingTop:'18px'}}>
          ...
          <div style={{display:'flex',height:'3px',width:'42px',borderRadius:'2px',overflow:'hidden'}}><div style={{flex:1,background:'#9CA3AF'}}/><div style={{flex:1,background:'#9CA3AF'}}/><div style={{flex:1,background:'#EF2118'}}/></div>
        </div>
      </footer>
    </main>
  )
