...
const supabase = createClient()
const {data} = await supabase.from('listings').select(PUBLIC_COLS).eq('status','active').eq('is_featured',true).gte('boost_expires_at',new Date().toISOString()).order('created_at',{ascending:false}).limit(6)
...
async function fetchListings() {
    ...
    let q = supabase.from('listings').select(u ? CONTACT_COLS : PUBLIC_COLS).ilike('status','active').order('created_at',{ascending:false})
