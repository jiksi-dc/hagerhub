...
@keyframes featuredshimmer { 0%,100%{box-shadow:0 0 0 rgba(218,165,32,0)} 50%{box-shadow:0 4px 20px rgba(218,165,32,0.15)} }
      ...
      <section className="hero-section" style={{position:'relative',width:'100%',minHeight:'340px',background:'#f9fafb',overflow:'hidden'}}>
        ...
        <div style={{maxWidth:'480px',flex:'0 0 auto',display:'flex',flexDirection:'column',alignItems:'flex-start'}}>
          <div className="cats-wrap" style={{display:'none'}}><div className="nav-cats">{TABS.map(tab=>{const on=activeCat===tab.name;return (<button key={tab.key} onClick={()=>setActiveCat(tab.name)} style={{fontSize:'16px',fontWeight:700,color:on?'#fff':'#6B7280',background:on?'#111':'transparent',padding:'9px 14px',border:'none',borderRadius:'999px',whiteSpace:'nowrap',flexShrink:0,fontFamily:'inherit',cursor:'pointer'}}>{(tab as any).label || tab.name}</button>)})}</div></div><div className="nav-mobilebtns" style={{display:'none'}}><AuthButton/><button onClick={()=>{setMobileFilters(o=>!o); if(aiOpen)setAiOpen(false)}} style={{flex:1,height:'44px',borderRadius:'10px',border:'none',background:'#111',color:'#fff',fontSize:'12px',fontWeight:700,cursor:'pointer',fontFamily:'inherit'}}>Search</button><button onClick={()=>{setAiOpen(o=>!o); if(mobileFilters)setMobileFilters(false)}} style={{flex:1,height:'44px',borderRadius:'10px',border:'1.5px solid #111',background:'#fff',color:'#111',fontSize:'12px',fontWeight:700,cursor:'pointer',fontFamily:'inherit'}}>AI Assistant</button><a href={`/${locale}/post`} style={{flex:1,height:'44px',display:'flex',alignItems:'center',justifyContent:'center',background:'#111',color:'#fff',borderRadius:'10px',fontSize:'12px',fontWeight:700,textDecoration:'none'}}>+ Post Ad</a></div><div className="hero-title" style={{textAlign:'left',marginBottom:'22px'}}>
      ...
      <footer style={{background:'#fff',borderTop:'1px solid #EBEBEB',padding:'32px 20px 24px',marginTop:'16px'}}>
        <div style={{maxWidth:'1280px',margin:'0 auto'}}>
          ...
          <div style={{borderTop:'1px solid #EBEBEB',paddingTop:'16px',display:'flex',justifyContent:'space-between',alignItems:'center',flexWrap:'wrap',gap:'8px'}}>
            <span style={{color:'#9CA3AF',fontSize:'12px'}}>© 2025 Ethiofy · Jiksi Michael</span>
