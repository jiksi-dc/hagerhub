...
export default function AdvertisePage() {
  ...
  const submit = async () => {
    ...
    try {
      ...
      const { error } = await supabase.from('inquiries').insert({ type: 'advertise', name, company, email, subject, message })
  ...
  return (
    <main style={{minHeight:'100vh',background:'#F9FAFB',fontFamily:'-apple-system,BlinkMacSystemFont,"Segoe UI",sans-serif'}}>
      ...
      <div style={{maxWidth:'560px',margin:'0 auto',padding:'48px 20px 80px'}}>
        <h1 style={{fontSize:'28px',fontWeight:900,color:'#111',marginBottom:'8px'}}>Advertise with Us</h1>
        <div style={{fontSize:'14px',color:'#6B7280',lineHeight:1.7,marginBottom:'28px'}}>Reach Ethiopia's most active marketplace audience. Tell us about your brand and goals, and our partnerships team will be in touch.</div>
        {status === 'sent' ? (
          ...
          <div style={{display:'flex',flexDirection:'column',gap:'18px'}}>
            ...
            <div><label style={lbl}>Company</label><input value={company} onChange={e=>setCompany(e.target.value)} style={field} placeholder="Your company or brand"/></div>
            ...
            <div><label style={lbl}>Subject</label><input value={subject} onChange={e=>setSubject(e.target.value)} style={field} placeholder="e.g. Banner placement, category sponsorship"/></div>
            <div><label style={lbl}>Message</label><textarea value={message} onChange={e=>setMessage(e.target.value)} style={{...field,minHeight:'120px',resize:'vertical'}} placeholder="Tell us about your budget, timeline, and what you'd like to promote."/></div>
