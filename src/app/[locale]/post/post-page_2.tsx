...
const [currency, setCurrency] = useState<'ETB'|'USD'>('ETB')
  ...
  useEffect(() => {
    ...
    supabase.auth.getUser().then(async ({ data }) => {
      if (!data.user) { router.push(`/login?next=${encodeURIComponent(window.location.pathname + window.location.search)}`); return }
      setUser(data.user)
      if (editId) {
  ...
  async function handleSubmit() {
    ...
    try {
      ...
      setStatus('✅ Approved! Checking your photos...')
      ...
      for (const photo of photos) {
        ...
        if (json.url) { image_urls.push(json.url) }
          ...
          setError(json.error || 'Photo upload failed. Please try another image.')
