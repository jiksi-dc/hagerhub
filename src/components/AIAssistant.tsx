  ...
  return (
    <>
      ...
      <button
        ...
        style={{position:'fixed',top:'12px',right:'12px',zIndex:9999,width:'48px',height:'48px',borderRadius:'50%',background:'#078754',border:'none',cursor:'pointer',boxShadow:'0 4px 20px rgba(0,87,217,0.3)',display:'flex',alignItems:'center',justifyContent:'center',fontSize:'19px',color:'#0057D9',fontWeight:900,transition:'transform 0.2s'}}
      ...
      {open && (
        <div style={{position:'fixed',top:'80px',right:'12px',zIndex:9999,width:'320px',height:'480px',background:'white',borderRadius:'20px',boxShadow:'0 8px 40px rgba(0,0,0,0.2)',display:'flex',flexDirection:'column',overflow:'hidden'}}>
          {/* Header */}
          <div style={{background:'#0057D9',padding:'16px',display:'flex',alignItems:'center',gap:'10px'}}>
            ...
            <div>
              <div style={{color:'#FCDD09',fontWeight:800,fontSize:'14px'}}>HagerHub AI</div>
          ...
          <div style={{flex:1,overflowY:'auto',padding:'12px',display:'flex',flexDirection:'column',gap:'8px'}}>
            {messages.map((m, i) => (
              <div key={i} style={{display:'flex',justifyContent:m.role==='user'?'flex-end':'flex-start'}}>
                <div style={{maxWidth:'80%',padding:'10px 14px',borderRadius:m.role==='user'?'18px 18px 4px 18px':'18px 18px 18px 4px',background:m.role==='user'?'#0057D9':'#F0F2F5',color:m.role==='user'?'white':'#111',fontSize:'13px',lineHeight:1.5}}>
                  {m.content}
                </div>
              </div>
            ))}
          ...
          <div style={{padding:'12px',borderTop:'1px solid #eee',display:'flex',gap:'8px'}}>
            ...
            <button
              onClick={send}
              disabled={loading}
              style={{width:'38px',height:'38px',borderRadius:'50%',background:'#0057D9',border:'none',cursor:'pointer',display:'flex',alignItems:'center',justifyContent:'center',fontSize:'16px',flexShrink:0}}>
              ➤
            </button>
          </div>
        </div>
