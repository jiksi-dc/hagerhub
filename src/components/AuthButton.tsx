...
export default function AuthButton() {
  ...
  if (!user) return (
    <a href="/login" style={{fontSize:'13px',fontWeight:600,padding:'8px 18px',background:'#FCDD09',color:'#111',borderRadius:'20px',textDecoration:'none'}}>
      Sign in
    </a>
  )
  ...
  return (
    <div style={{position:'fixed',top:'12px',left:'12px',zIndex:1000}}>
      <button onClick={()=>setMenuOpen(o=>!o)}
        style={{width:'36px',height:'36px',borderRadius:'50%',background:'#FCDD09',color:'#111',fontSize:'13px',fontWeight:700,border:'none',cursor:'pointer'}}>
        {initials}
      </button>
      {menuOpen && (
