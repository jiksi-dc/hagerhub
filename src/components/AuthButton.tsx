...
export default function AuthButton() {
  ...
  if (!user) return (
    <a href="/login" style={{fontSize:'13px',fontWeight:600,padding:'8px 18px',background:'#60A5FA',color:'white',borderRadius:'20px',textDecoration:'none'}}>
      Sign in
    </a>
  )
  ...
  return (
    <div style={{position:'fixed',top:'12px',left:'12px',zIndex:1000}}>
      <button onClick={()=>setMenuOpen(o=>!o)}
        style={{width:'36px',height:'36px',borderRadius:'50%',background:'#60A5FA',color:'white',fontSize:'13px',fontWeight:700,border:'none',cursor:'pointer'}}>
        {initials}
      </button>
      {menuOpen && (
